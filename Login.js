...
function Login() {
    //change the URL after sign in
    const history = useHistory();
    //create the variables to capture login info
    ...
    const signIn = e => {
        ...
        //if sign in is successful, push to the home page
        .then(auth => {
            history.push ('/')
        })
    ...
    const register = e => {
        ...
        .then((auth) => {
            ...
            //if auth is not empty (user input), redirect home
            if (auth) {
                history.push('/')
            }
        })
